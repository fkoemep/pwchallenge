...
def load(data):
    ...
    try:
        with engine.begin() as conn:
            ...
            # dim_doctor
            print('Loading the following dim_doctor records to Hydra...')
            print(data['dim_doctor'].to_json(orient='records', lines=True))

            data['dim_doctor'].to_sql('dim_doctor', conn, if_exists="append", index=False, method=insert_on_conflict_nothing)
            ...
            # dim_test
            print('Loading the following dim_test records to Hydra...')
            print(data['dim_test'].to_json(orient='records', lines=True))

            data['dim_test'].to_sql('dim_test', conn, if_exists="append", index=False, method=insert_on_conflict_nothing)

            # fact_hospital_events
            print('Loading the following fact_hospital_events records to Hydra...')
            print(data['fact_hospital_events'].to_json(orient='records', lines=True))

            data['fact_hospital_events'].to_sql('fact_hospital_events', conn, if_exists="append", index=False)
